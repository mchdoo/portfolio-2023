...
import RenderImage from "../../../components/renderImage";
 ...
 async function Galeria() {
   ...
   const {items: renders} = await client.getAssets({"metadata.tags.sys.id[in]": ["galeria"] })

  return (
    <main>
      <Marquee className="border-y py-2 bg-accent-2/20 border-fore uppercase" autoFill>
        Preview – página en contrucción –&nbsp;
      </Marquee>
      
      <section className="grid grid-flow-row grid-cols-2 md:grid-cols-3 lg:grid-cols-4 auto-rows-min">
        {renders.map((render, index) => <RenderImage key={index} render={render} />)}
      </section>
